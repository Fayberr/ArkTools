...
Required environment variables for `trusted-signing-cli` mode (prefixes can be `ARKTOOLS_` or legacy `BLUR_`):

- `ARKTOOLS_WINDOWS_SIGNING_MODE=trusted-signing-cli`
- `ARKTOOLS_TRUSTED_SIGNING_ENDPOINT`
- `ARKTOOLS_TRUSTED_SIGNING_ACCOUNT`
- `ARKTOOLS_TRUSTED_SIGNING_PROFILE`
- `ARKTOOLS_TRUSTED_SIGNING_DESCRIPTION` optional, defaults to `ArkTools`
- `AZURE_CLIENT_ID`
- `AZURE_CLIENT_SECRET`
- `AZURE_TENANT_ID`
...
If `ARKTOOLS_WINDOWS_SIGNING_MODE` is unset or set to `none`, the wrapper exits successfully without signing so the build can still complete.
...
```powershell
Get-AuthenticodeSignature src-tauri\target\release\bundle\nsis\ArkTools_1.5.7_x64-setup.exe
```

Check the built executable:

```powershell
Get-AuthenticodeSignature src-tauri\target\release\ArkTools.exe
```
